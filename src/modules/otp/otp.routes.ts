...
const sendOtpSchema = z.object({
  ...
  otp: z
    ...
    .optional(),
  ttl_seconds: z.coerce.number().min(60).max(3600).default(300),
  template_name: z.string().default("otp_auth_v1"),
...
const resendOtpSchema = z.object({
  request_id: z.string().min(1, { message: "Unique OTP request ID to resend" }),
});
...
export const otpRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/:request_id", async (request, reply) => {
