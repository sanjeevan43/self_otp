...
export const walletRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/topup", { preHandler: fastify.authenticateJwt }, async (request, reply) => {
  ...
  fastify.get("/transactions", { preHandler: fastify.authenticateJwt }, async (request, reply) => {
    const customer = request.customer!;
    const wallet = await WalletService.getOrCreateWallet(customer.id);
